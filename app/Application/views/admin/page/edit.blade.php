@extends(layoutExtend())
 @section('title')
  {{  isset($item) ? trans('home.edit')  : trans('home.add')  }}   {{ trans('page.page') }}
@endsection
@section('style')
 ...
 @section('content')
    @component(layoutForm() , ['title' => trans('page.page') , 'model' => 'page' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form action="{{ concatenateLangToUrl('admin/page/item') }}{{ isset($item) ? '/'.$item->id : '' }}"
            ...
            <div class="form-group {{  $errors->has("slug")  &&  $errors->has("slug")  ? "has-error" : "" }}">
               <label for="slug">{{ trans("page.slug")}}</label>
               <input type="text" name="slug" class="form-control" id="slug" value="{{ isset($item->slug) ? $item->slug : old("slug") }}"  placeholder="{{ trans("page.slug")}}">
                @if ($errors->has("slug"))
                    <div class="alert alert-danger">
                           <span class='help-block'>
