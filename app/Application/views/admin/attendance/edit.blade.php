...
@section('content')
    @component(layoutForm() , ['title' => trans('attendance.attendance') , 'model' => 'attendance' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('admin/attendance/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
		 ...
		 <div class="form-group  {{  $errors->has("notes.ar")  &&  $errors->has("notes.en")  ? "has-error" : "" }}" >
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('attendance.attendance') }}
                </button>
            </div>
        </form>
    @endcomponent
