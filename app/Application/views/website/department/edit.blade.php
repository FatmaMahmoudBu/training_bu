@extends(layoutExtend('website'))
 @section('title')
    {{ trans('department.department') }} {{  isset($item) ? trans('home.edit')  : trans('home.add')  }}
@endsection
 @section('content')
<div class="col-lg-12">
         @include(layoutMessage('website'))
         <a href="{{ url('department') }}" class="btn btn-danger"><i class="fa fa-arrow-left"></i> {{ trans('website.Back') }}  </a>
        <form action="{{ concatenateLangToUrl('department/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("website.department.relation.faculty.edit")
   ...
   @endif
             <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('website.Update') }}  {{ trans('website.department') }}
                </button>
            </div>
        </form>
</div>
